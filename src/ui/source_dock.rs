...
use crate::ui::filmstrip_background::FilmFrame;
use crate::ui::theme::{self, MUTED, TEXT};
use crate::ui::timeline::{ExpandedAudio, LayerFlags, QncTimeline, TimelineFocusPaint};
use crate::ui::timeline_progress::{
...
pub fn dock_height(expanded_audio: ExpandedAudio, show_header: bool) -> f32 {
    ...
    if show_header {
        track + theme::CHROME_ROW_H + HEADER_TIMELINE_GAP_H
    } else {
        track
    }
}
...
pub fn show(ui: &mut egui::Ui, input: SourceDockInput<'_>) -> SourceDockAction {
    ...
    let tl_colors = theme::current(ui).timeline();
    ...
    egui::Frame::NONE
        ...
        .show(ui, |ui| {
                ...
                theme::chrome_row(ui, true, |ui| {
                    if input.show_edit_actions {
                        ...
                        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                            if theme::action_btn(ui, "Pokrivalice").clicked() {
                                action = SourceDockAction::CreateCover;
                            }
                            if theme::action_btn(ui, "Voice over").clicked() {
                                action = SourceDockAction::CreatePart("offovi");
                            }
                            if theme::action_btn(ui, "Talking Head").clicked() {
                                action = SourceDockAction::CreatePart("tonovi");
                            }
                            if theme::action_btn(ui, "Add virtual clip").clicked() {
                        ...
                        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                            ...
                            if theme::action_btn(ui, "Osvježi").clicked() {
                            ...
                            let ingest = ui.add_enabled_ui(input.import_enabled, |ui| {
                                theme::primary_btn(ui, "Ingest")
                            });
                            if ingest.inner.clicked() {
                                action = SourceDockAction::ImportSelected;
                            }
                            if theme::action_btn(ui, "Odaberi sve").clicked() {
                                action = SourceDockAction::SelectAll;
                            }
                            if theme::action_btn(ui, "Očisti").clicked() {
                                action = SourceDockAction::ClearSelection;
                            }
                            if input.archive_original_available {
                                if ui
                                    .checkbox(
                                        &mut archive,
                                        RichText::new("Kopiraj original").size(theme::FONT_UI),
                                    )
                                    .changed()
                                {
                            ...
                            if ui
                                .checkbox(
                                    &mut ai_mining,
                                    RichText::new("AI mining").size(theme::FONT_UI),
                                )
                                .changed()
                            {
            ...
            let filmstrip_background = |ui: &mut egui::Ui, rect: egui::Rect| {
                crate::ui::filmstrip_background::paint(ui, rect, input.frames);
            };
            ...
            let intent = timeline_progress::show(
                ui,
...
fn timecode_label(ui: &mut egui::Ui, label: &str, value: String, focused: bool) {
    theme::timecode_label(ui, label, &value, focused);
}
