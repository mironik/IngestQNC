...
pub mod css {
    ...
    pub const PENDING_COVER: Color32 = Color32::from_rgb(251, 146, 60);
...
fn paint_covers(
    ...
    for cover in covers {
        ...
        let fill = if cover.pending {
            css::PENDING_COVER.linear_multiply(if cover.selected { 0.85 } else { 0.55 })
        ...
        if cover.pending {
            painter.rect_stroke(
                rect,
                2.0,
                egui::Stroke::new(if cover.selected { 2.0 } else { 1.2 }, css::PENDING_COVER),
                egui::StrokeKind::Inside,
            );
            if rect.width() >= 56.0 {
                painter.text(
