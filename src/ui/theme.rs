...
fn theme_id_key() -> egui::Id {
    egui::Id::new("ingest_qnc_theme_id")
}

pub fn set_active(ctx: &egui::Context, id: ThemeId) {
    ctx.data_mut(|data| data.insert_temp(theme_id_key(), id));
}

pub fn active_id(ctx: &egui::Context) -> ThemeId {
    ctx.data(|data| data.get_temp::<ThemeId>(theme_id_key()).unwrap_or_default())
}
...
pub fn transport_btn(ui: &mut egui::Ui, label: &str) -> egui::Response {
    let tokens = current(ui);
    let width = match label.chars().count() {
        0..=2 => 40.0,
        3..=8 => 86.0,
        _ => 124.0,
    };
    ui.add(
        egui::Button::new(RichText::new(label).color(tokens.text).size(FONT_UI))
...
pub fn link_tab(ui: &mut egui::Ui, label: &str, active: bool) -> egui::Response {
    ...
    let text = if active {
        ...
        RichText::new(label).color(tokens.muted).size(FONT_UI)
    };
    let response = ui.add(
        egui::Label::new(text)
            .sense(Sense::click())
...
pub fn text_link(ui: &mut egui::Ui, label: &str, enabled: bool) -> egui::Response {
    let tokens = current(ui);
    let color = if enabled { tokens.text } else { tokens.muted };
    ui.add_enabled(
        enabled,
...
pub fn timecode_label(ui: &mut egui::Ui, label: &str, value: &str, focused: bool) {
    let tokens = current(ui);
    let label_color = if focused { tokens.focus } else { tokens.muted };
